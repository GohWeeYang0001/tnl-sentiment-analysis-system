...
try:
    ...
    st.error("Model loading failed. Please make sure svm_linearsvc_model.pkl and tfidf_vectorizer.pkl are in the same folder as app.py.")
    st.exception(e)
    st.stop()
...
with tab1:
    ...
    if st.button("Predict Sentiment", key="single_predict_button"):
        if user_comment.strip() == "":
            ...
            st.caption(
                "LinearSVC does not directly output probability scores, so the system displays the predicted sentiment label instead of confidence percentage."
            )
